...
//이메일 넣을때 이상하게 넣는거 방지? 전화번호도?
//에러 찾기
//일반 회원 6번 개인정보수정까지 확인 마침. 7번부터 확인할것

//admin 접속해서 도서 추가할때 integer변환시 에러(숫자에 문자들어가면) NumberFormatException 이거 예외처리하기부터 시작
...
public class LibraryStart {
	...
	TreeMap<String, String> list4 = new TreeMap<String, String>();
	List<MemberVO> data2 = new  ArrayList<MemberVO>(); //중복검사
	List<BookListVO> data4 = new  ArrayList<BookListVO>();	//대출현황 //지워야?
...
//								}
							...
							do {   
								...
								if(menu3.equals("1")) { //1. 도서목록 호출
									...
									bookListInsert();
								}else if(menu3.equals("4")) { //4. 도서관 대출현황
									for(String i : list3.keySet()) {
										System.out.println(i+list3.get(i));
									}
								}else if(menu3.equals("5")) { //5. 회원목록
									memberList2();
								}else if(menu3.equals("6")) { //6. 회원삭제
									memberDelete2();
									if(ex==1) {
										cnt--;
										break a;
									}
								}else if(menu3.equals("7")) { //7. 로그아웃
									break a;
								}else if(menu3.equals("8")) { //8. 종료
									System.exit(0);
								}else {
									System.out.println("잘못 입력하셨습니다.");
								}
...
//				
			...
			if(vo!=null) {
				...
				do {
					 ...
					 EmailCheckEX check = new EmailCheckEX();
			...
			if(vo!=null) {
				String rent = conInput("대출하시겠습니까?(1:예, 2:아니오)");
					if(rent.equals("1")) {
						...
						list3.put("대여번호 : "+treenum2+"번  ["+treenum+"]", " "+loginid +"님이 "+"<"+msg+">"+"도서를 대여하셨습니다."+"반납 예정 시간은["+treenum3+"]까지 입니다.");
						treenum2++;
						System.out.println("------------------------------------------------");
		...
		for(int i=0; i<mybook; i++) {
				if(msg.equals(data4.get(i).getName())) {
					if("대여중".equals(data4.get(i).getRental())) {
						System.out.println("회원님이 대여하신 도서입니다.");
						return true;
					}
...
//		String today2 = sdf2.format(now.getTime());
